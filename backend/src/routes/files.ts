...
// List directory
files.get("/:id/files", requireAuth, (c) => {
...
// Download file
files.get("/:id/files/download", requireAuth, (c) => {
  const { id } = c.req.param();
  const server = serverQueries.getById.get(id);
  if (!server) return c.json({ error: "Server not found" }, 404);
...
files.post("/:id/files/upload", requireAuth, async (c) => {
  const { id } = c.req.param();
  const server = serverQueries.getById.get(id);
  if (!server) return c.json({ error: "Server not found" }, 404);
...
files.delete("/:id/files", requireAuth, (c) => {
  const { id } = c.req.param();
  const server = serverQueries.getById.get(id);
  if (!server) return c.json({ error: "Server not found" }, 404);
...
// Create directory
files.post("/:id/files/mkdir", requireAuth, (c) => {
  const { id } = c.req.param();
  const server = serverQueries.getById.get(id);
  if (!server) return c.json({ error: "Server not found" }, 404);

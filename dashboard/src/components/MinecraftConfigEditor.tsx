            ...
            {platform.fields.map((f) => (
              <div key={f.key}>
                ...
                <div className="relative">
                  <input
                    type={f.type === "password" && !showApiKey ? "password" : "text"}
                    value={envVars[f.key] ?? ""}
                    placeholder={f.placeholder}
                    onChange={(e) => set(f.key, e.target.value)}
                    className="w-full bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-sm font-mono text-white focus:outline-none focus:border-brand-500 pr-10"
                  />
                  {f.type === "password" && (
                    <button
                      type="button"
